...
def test_loop(local_rank: int, num_local_ranks: int, args: argparse.Namespace):
    ...
    if args.test_ll_compatibility:
        num_rdma_bytes = deep_ep.Buffer.get_low_latency_rdma_size_hint(max(ll_tokens), args.hidden, num_ranks, args.num_experts)
        if local_rank == 0:
        ...
        for num_tokens in ll_tokens:
            ...
            test_low_latency.test_main(num_tokens, args.hidden, args.num_experts, args.num_topk, rank, num_ranks, group, buffer, seed=1)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--disable-nvlink', action='store_true',
                        help='whether to disable NVLink')
    parser.add_argument('--allow-mnnvl', action='store_true',
                        help='whether to allow MNNVL')
