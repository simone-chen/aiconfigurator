...
def test_main(num_tokens: int, hidden: int, num_experts: int, num_topk: int,
    ...
    for return_recv_hook in (True, False):
        ...
        if not return_recv_hook:
            if rank == 0:
                print(f'[rank {rank}] num_tokens={num_tokens}, hidden={hidden}, num_experts={num_experts}, num_topk={num_topk}, return_recv_hook={return_recv_hook} Dispatch bandwidth: {num_dispatch_comm_bytes / 1e9 / dispatch_t:.2f} GB/s, avg_t={dispatch_t * 1e6:.2f} us | '
                        f'Combine bandwidth: {num_combine_comm_bytes / 1e9 / combine_t:.2f} GB/s, avg_t={combine_t * 1e6:.2f} us', flush=True)
        else:
            if rank == 0:
                print(f'[rank {rank}] num_tokens={num_tokens}, hidden={hidden}, num_experts={num_experts}, num_topk={num_topk}, return_recv_hook={return_recv_hook} Dispatch send/recv time: {dispatch_t[0] * 1e6:.2f} + {dispatch_t[1] * 1e6:.2f} us | '
