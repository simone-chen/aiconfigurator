                ...
                for model_config in model_config_list:
                    ...
                    for tp in tp_list:
                        for ep in ep_list:
                            ...
                            #test_cases.append([moe_type,num_token,hs,inter_s,topk,num_experts,tp,ep,False,model_name,'moe_perf.txt',"uniform",0])
    return test_cases
...
def benchmark_config(
    ...
    if distributed == "uniform":
        ...
        gating_output = [power_law_logits_v3(num_tokens, num_experts, topk, 1, power_law_alpha) for _ in range(num_iters)]
    else:
...
def run_moe_torch(moe_type, num_tokens, hidden_size, inter_size, topk, num_experts, moe_tp_size, moe_ep_size, cutlass_min_latency_mode, model_name, perf_filename, distributed = "power_law", power_law_alpha = 0, device='cuda:0'):
    torch.cuda.set_device(device)
    torch.set_default_device(device)

    assert moe_ep_size == 1, f"only support moe ep size = 1"
    assert moe_type == "fp8_block" or moe_type == "float16", f"only support moe type = fp8_block or float16"
    assert inter_size % moe_tp_size == 0, f"inter_size % moe_tp_size must be 0"

    latency = benchmark(num_tokens, num_experts, 2 * inter_size // moe_tp_size, hidden_size, topk, torch.bfloat16, moe_type == "fp8_block", False, False, None, distributed=distributed, power_law_alpha=power_law_alpha)
